    ...
    >
      <div className="bg-white dark:bg-slate-900 rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-4 my-8">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">Configure Action Groups</h3>
          <div className="flex gap-2">
        ...
        <div className="space-y-4">
          {Object.entries(actionGroups).map(([gk, group]) => (
            <div key={gk} className="border border-gray-300 dark:border-slate-700 p-3 rounded">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="font-semibold text-gray-800 dark:text-gray-100 dark:bg-slate-800 px-2 py-1 border border-gray-300 dark:border-slate-600 rounded"
                  />
                </div>
                <button 
                  onClick={() => onDeleteGroup(gk)} 
                  className="w-8 h-8 flex items-center justify-center rounded hover:bg-gray-100 dark:hover:bg-slate-700 text-red-600 hover:text-red-700 transition"
                  title="Delete group"
                >
              ...
              <div className="space-y-2">
                {group.actions.map((act, idx) => (
                  <div key={act.id + idx} className="border border-gray-200 dark:border-slate-600 p-2 rounded bg-white dark:bg-slate-800">
                    <div className="flex gap-2 items-start mb-2">
                      <input
                        value={act.id}
                        onChange={(e) => onUpdateActionInGroup(gk, idx, { id: e.target.value })}
                        className="px-2 py-1 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded w-32 text-sm"
                        placeholder="ID"
                      />
                      <input
                        value={act.label}
                        onChange={(e) => onUpdateActionInGroup(gk, idx, { label: e.target.value })}
                        className="px-2 py-1 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded flex-1 text-sm"
                        placeholder="Label"
                      />
                      <button
                        onClick={() => onDeleteActionInGroup(gk, idx)}
                        className="w-8 h-8 flex items-center justify-center rounded hover:bg-gray-200 dark:hover:bg-slate-700 text-red-600 hover:text-red-700 transition"
                        title="Delete action"
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                          ...
                          return (
                            <div key={configIdx} className="flex gap-2 items-center">
                              ...
                              <select
                                ...
                                className="px-2 py-1 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded w-20 text-xs"
                              >
                                <option value="number">Number</option>
                                <option value="text">Text</option>
                              </select>
                              <input
                                ...
                                className="px-2 py-1 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 rounded flex-1 text-xs"
